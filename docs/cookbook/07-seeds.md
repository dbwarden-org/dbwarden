...
Define `forward(connection, session)` and `reverse(connection, session)` for programmatic data:
...
Seed rollback executes the configured inverse operation and removes the tracking record only after it succeeds. SQL file seeds require a `-- rollback` section; Python and procedural code seeds require `reverse(connection, session)`.
...
- **Code seeds (`Seed` base class) are the recommended approach**: immutable explicit versions, full IDE support, stays in sync with models
...
- Seed rollback executes the seed's explicit reverse operation
