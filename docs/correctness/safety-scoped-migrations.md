...
UNKNOWN blocks under every ceiling, including the default CRITICAL ceiling:
a file whose plan resolves to explicit UNKNOWN (parse failure, tamper
evidence, tagged UNKNOWN) stops with exit 3. The only exception is a file
with no plan at all, which is governed by the missing_plan preflight policy
and passes at the default ceiling. Applied historical files are not
reclassified. There is no per-file severity override.
